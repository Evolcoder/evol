# Space Game ("Star Wars")
# by EvolGames : Harsh
# This game has so many features 
# like there are three types of enemies in it.
# there is a powerup energy from which you can heal
# There is a radar and level, lives show panel
...
wn.title("Star Wars! by @EvolGames")
wn.bgcolor("black")
wn.tracer(0)
...
class Game():
    ...
    def render_info(self, pen, score, active_enemies = 0):
        ...
        character_pen.draw_string(pen, "STAR WARS", 400, 270)
...
# Splash screen
character_pen = CharacterPen("red", 3.0)
character_pen.draw_string(pen, "STAR WARS", 0, 160)

character_pen.scale = 1.0
character_pen.draw_string(pen, "BY EvolGames:Harsh", 0, 100)
...
class Player(Sprite):
    ...
    def rotate_left(self):
        self.da = 7 # Change the speed of spacecraft
        
    def rotate_right(self):
        self.da = -7  # changes in anticlock direction
    ...
    def fire(self):
        ...
        for missile in missiles:
            if missile.state == "ready":
                #missile.fire(self.x, self.heading, self.dx, self. dy)
                #break
                num_of_missiles += 1
        ...
        if num_of_missiles == 2:
            directions = [-5, 5] # spread the missiles
            for missile in missiles:
                if missile.state == "ready":
                    missile.fire(self.x, self.y, self.heading + directions.pop(), self.dx, self. dy)
        ...
        if num_of_missiles == 3:
            directions = [0, -5, 5]  # spread the missiles
            for missile in missiles:
                if missile.state == "ready":
                    missile.fire(self.x, self.y, self.heading +directions.pop(), self.dx, self. dy)
...
class Missile(Sprite):
    def __init__(self, x, y, shape, color):
        ...
        self.state = "ready"
        self.thrust = 8.0 # Change the speed of thrust
        self.max_fuel = 200
...
missiles = []
for _ in range(3):  # Boost the no. of missiles
